...
\begin{frame}[t]{Totaler kausaler Effekt: Äquivalenzen}
    ...
    \only<11-13>{
        ...
        \onslide<12->{(ii) $\overset{A.1}{\Rightarrow} \exists x_1^\triangle, x_1^\square$ mit pos. Dichte unter $\hat{N_1}$ sodass $\mathbb{P}_\mathcal{S}^{X_2|X_1=x_1^\triangle, do(X_1=\hat{N_1})} \neq \mathbb{P}_\mathcal{S}^{X_2 | X_1 = x_1^\square, do(X_1 = \hat{N_1})}$}
    ...
    \only<15-17>{
        \textbf{Beweisplan:} $\neg$(i) $\Rightarrow$ $\neg$ (iii)\\
        \onslide<16->{Es gilt: $\mathbb{P}_\mathcal{S}^{X_2} = \mathbb{P}_\mathcal{S}^{X_2 | do(X_1 = N_1^*)}$, wobei $N_1^*$ wie $\mathbb{P}_\mathcal{S}^{X_2}$ verteilt ist.\\}
        \onslide<17->{
            \begin{align}
                \neg (i) &\Rightarrow X_2 \perp\!\!\!\perp X_1 \text{ in } \mathbb{P}_\mathcal{S}^{\textbf{X}|do(X_1 = N_1^*)}\\
                &\overset{A.3}{\Rightarrow} \mathbb{P}_\mathcal{S}^{X_2| X_1=x^\triangle | do(X_1=N_1^*)} = \mathbb{P}_\mathcal{S}^{X_2 | do(X_1 = N_1^*)} \;\;\;\forall x^\triangle \text{ mit } p_1(x^\triangle) > 0\\
                &\overset{A.1}{\Rightarrow} \mathbb{P}_\mathcal{S}^{X_2 | do(X_1=x^\triangle)} = \mathbb{P}_\mathcal{S}^{X_2} \;\;\; \forall x^\triangle \text{ mit } p_1(x^\triangle) > 0\\
                &\overset{\neg (ii)}{\Rightarrow} \neg (iii)
            \end{align}
        }
    }
